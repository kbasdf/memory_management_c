...
void register(int pointer, char *char_ptr, char *char_f,int *c_count){
...
int compress(int n, int *ptr1,int *ptr2, int *extra_ptr){
 int boundary_r;
 int boundary_l;
 ...
 char characters[how_many] = {};
 char characters_flag[how_many] = {};
 int characters_count[how_many] = {};
 char *c_ptr, *c_ptr_f;
 int *c_count;
 char *ptr_abt_loc[2];
 char *ptr_location=NULL;
 char *ptr_next=NULL;
 *
 *ptr_location=NULL;
 *ptr_next=
 c_ptr=&characters;
 c_ptr_f=&charactdrs_flag;
 ...
 while(pointer <= boundary_l && memory[pointer] != '\0'){
        ...
        // that's it....
        register(pointer,c_ptr,c_ptr_f,c_count);
        char *location;
        location=malloc(16);
        *(location)=&
        *(location+8)=
    ...
    if(pointer == cluster_coordinates[n][0]){
            ...
            pointer=cluster_coordinates[n][1]-1;
            goto here;
        }
...
int optimize_l(int n,int code){
    ...
    if (code ==0){
        ...
        compress(n,ptr1,ptr2,extra_ptr);
